...
<div class="card card-outline rounded-0 card-navy">
	<div class="card-header">
		<h3 class="card-title">Daily Recipient Report</h3>
	</div>
	<div class="card-body">
        ...
        <div class="container-fluid" id="printout">
			<table class="table table-hover table-striped table-bordered" id="list">
				...
				<tbody>
					...
					$i = 1;
					$total = 0;
						$qry = $conn->query("SELECT r.*, c.name as category, t.name as `toll` from `recipient_list` r inner join category_list c on r.category_id = c.id inner join `toll_list` t on r.toll_id = t.id where date(r.`date_created`) = '{$date}' order by unix_timestamp(r.`date_created`) desc ");
					...
					<?php if($qry->num_rows <= 0): ?>
						<tr>
							<th colspan="6" class="text-center"><b>No Data</b></th>
						</tr>
					<?php endif; ?>
				</tbody>
				<tfoot>
		...
		<div class="d-flex w-100 align-items-center">
			...
			<div class="col-8 text-center">
				<div style="line-height:.7em">
					...
					<h3 class="text-center mb-0">Daily Recipient Report</h3>
...
<script>
	$(document).ready(function(){
		...
		$('#filter').submit(function(e){
			e.preventDefault()
			location.href = "./?page=reports/recipient&"+$(this).serialize()
		})
	})
</script>