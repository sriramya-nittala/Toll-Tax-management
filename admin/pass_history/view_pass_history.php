...
if(isset($_GET['id']) && $_GET['id'] > 0){
    $qry = $conn->query("SELECT ph.*, p.code, p.vehicle_name, p.owner, p.vehicle_registration, p.vehicle_name, c.name as category, t.name as `toll` from `pass_history` ph inner join `pass_list` p on ph.pass_id = p.id inner join category_list c on p.category_id = c.id inner join toll_list t on ph.toll_id = t.id where ph.id = '{$_GET['id']}'");
    if($qry->num_rows > 0){
        ...
        echo '<script> alert("Pass History ID is invalid."); location.replace("./?page=pass_history");</script>';
    }
}else{
    echo '<script> alert("Pass History ID is required."); location.replace("./?page=pass_history");</script>';
}
?>
<div class="mx-0 py-5 px-3 mx-ns-4 bg-gradient-primary">
	<h3><b>Pass History Details</b></h3>
</div>
<style>
	img#cimg{
      max-height: 15em;
	...
	<div class="col-lg-10 col-md-11 col-sm-11 col-xs-11">
		<div class="card rounded-0 shadow">
			<div class="card-body">
				<div class="container-fluid" id="printout">
					<div class="row w-100">
						...
						<div class="col-8 border py-2 m-0 font-weight-bolder"><?= isset($toll) ? ($toll) : '0.00' ?></div>
			...
			<div class="card-footer py-1 text-center">
				...
				<a class="btn btn-primary btn-sm bg-gradient-primary rounded-0" href="./?page=pass_history/manage_pass_history&id=<?= isset($id) ? $id :'' ?>"><i class="fa fa-edit"></i> Edit</a>
				...
				<a class="btn btn-light btn-sm bg-gradient-light border rounded-0" href="./?page=pass_history"><i class="fa fa-angle-left"></i> Back to List</a>
			</div>
		</div>
	</div>
</div>
...
<script>
	...
	$(document).ready(function(){
        $('#delete-data').click(function(){
			_conf("Are you sure to delete this pass permanently?","delete_pass_history",['<?= isset($id) ? $id : '' ?>'])
		})
		...
		$.ajax({
			...
			success:function(resp){
				if(typeof resp== 'object' && resp.status == 'success'){
					location.replace("./?page=pass_history");
				}else{
					alert_toast("An error occured.",'error');
					end_loader();
				}
